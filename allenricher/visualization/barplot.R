...
# read data
plottype <- args[1]
rt <- read.table(args[2], sep="\t", quote="", comment.char="")
...
if(nrow(dat)>20){
    ...
    png(file=paste(args[3],"top20.png",sep="."),width=1000,height=800,res=100)
    dat <- dat[c(1:20),]
}else{
    pdf(file=paste(args[3],"pdf",sep="."),width=10)
    png(file=paste(args[3],"png",sep="."),width=1000,height=800,res=100)
}
num <- nrow(dat)
