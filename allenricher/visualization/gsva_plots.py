...
def plot_pathway_heatmap(
    ...
    参数:
        ...
        cmap: 颜色映射名称，默认使用 PlotTheme.get_diverging_cmap()
        center: 颜色映射中心值，默认 0
        title: 图表标题
    ...
    with PlotTheme.context(style or 'nature'):
        ...
        # 构建颜色映射字典（用于分组注释）
        palette_colors = PlotTheme.get_palette(palette, n=8)
        ...
        if output_file is not None:
            save_figure_dual(g.fig, output_file, dpi=dpi, facecolor="white")
...
def plot_group_comparison(
    ...
    with PlotTheme.context(style or 'nature'):
        ...
        # 保存文件
        if output_file is not None:
            save_figure_dual(fig, output_file, dpi=dpi, facecolor="white")
...
def plot_pathway_dotplot(
    ...
    with PlotTheme.context(style or 'nature'):
        ...
        if groups is not None and len(groups) > 0:
            ...
            # 使用 palette 获取颜色
            dot_colors = PlotTheme.get_palette(palette, n=len(top_pathways))
            ax.scatter(
