...
<body>
        ...
        for database, frame in results.items():
            ...
            for _, row in frame.iterrows():
                ...
                if term_id:
                    ...
                    safe_database = re.sub(r'[^A-Za-z0-9_.-]+', '_', str(database))
                    safe_term_id = re.sub(r'[^A-Za-z0-9_.-]+', '_', term_id)
                    term_names[term_id] = identity
                    term_names[safe_term_id] = identity
                    term_names[f'{safe_database}_{safe_term_id}'] = identity
    ...
    def _generate_ai_section(self, ai_interpretation: Dict[str, Any]) -> str:
        ...
        if ai_interpretation.get("schema_version") == 1:
            ...
            def render_item(item: Dict[str, Any]) -> str:
                ...
                for key, label in (
                    ...
                    value = item.get(key)
                    if value:
                        details.append(f'{label}: {self._html_text(str(value).replace("_", " "))}')
                shared_genes = item.get("shared_core_genes", [])
                if shared_genes:
                    shared_count = item.get("shared_core_gene_count", len(shared_genes))
                    details.append(
                        f'Shared core genes (n={self._html_text(shared_count)}): '
                        f'{self._html_text(", ".join(map(str, shared_genes[:10])))}'
                    )
                if item.get("auto_linked_evidence_ids"):
                    details.append(
                        f'Citation links completed by code: {len(item["auto_linked_evidence_ids"])}'
                    )
                detail_markup = (
                    f' <span class="ai-note">({"; ".join(details)})</span>' if details else ""
                )
                evidence_markup = (
                    f' <span class="ai-evidence">[{links}]</span>' if links else ""
                )
                return f'<li><span>{text}</span>{detail_markup}{evidence_markup}</li>'
            ...
            overall_items = ai_interpretation.get("overall_synthesis", [])
            research_items = ai_interpretation.get("research_summary", [])
            research_block = ""
            if research_items:
                research_block = (
                    '<div class="ai-block ai-research-summary"><h3>Research summary</h3><ul>'
                    + "".join(render_item(item) for item in research_items)
                    + '</ul></div>'
                )
            overall_block = ""
            if overall_items:
                overall_block = (
                    '<div class="ai-block ai-overall"><h3>Evidence pattern</h3><ul>'
                    + "".join(render_item(item) for item in overall_items)
                    + '</ul></div>'
                )
            database_blocks = []
            for database, content in ai_interpretation.get("databases", {}).items():
                ...
                for section, label in section_labels.items():
                    ...
                    entries = [render_item(item) for item in items]
                    if not entries:
        ...
        <div class="section" id="ai-interpretation">
            <h2>AI Interpretation</h2>
            <p class="ai-section-meta">Model: {backend} | Profile: {mode}; requires expert review</p>
            {research_block}
            {overall_block}
        ...
        <div class="section" id="ai-interpretation">
            <h2>AI Interpretation</h2>
            <p class="ai-section-meta">Requires expert review</p>
