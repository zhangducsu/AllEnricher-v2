...
def test_plot_identity_uses_database_namespace_for_shared_term_ids(tmp_path):
    """GSEA single-term plots must not collapse equal term IDs across databases."""
    generator = ReportGenerator(str(tmp_path))
    results = {
        "GO": pd.DataFrame({
            "Term_ID": ["SHARED"],
            "Term_Name": ["GO shared pathway"],
        }),
        "CustomDB": pd.DataFrame({
            "Term_ID": ["SHARED"],
            "Term_Name": ["Custom shared pathway"],
        }),
    }

    assert generator._plot_identity("GO_SHARED_enrichment", results, "gsea") == (
        "GO",
        "GO shared pathway (SHARED) - GSEA Enrichment Plot",
    )
    assert generator._plot_identity("CustomDB_SHARED_enrichment", results, "gsea") == (
        "CustomDB",
        "Custom shared pathway (SHARED) - GSEA Enrichment Plot",
    )
