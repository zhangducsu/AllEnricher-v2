#!/usr/bin/env python3
"""
A precise simulation of the logic of the calculation of v1
"""
...
# Statistical Background_total (genesis read from GO files, but only counted in Background_list)
background_total = 0
for term_id, term_info in go_data.items():
    for gene in term_info['genes']:
        if gene in background_list:
            background_total += 1
            break  # Every gene counts only once.

# In fact, the blackground_ttal should be the only gene in the Go file.
go_genes = set()
for term_id, term_info in go_data.items():
    for gene in term_info['genes']:
        go_genes.add(gene)
...
# Analogical logic of simulation v1
term_id = 'GO:0051301'
term_info = go_data[term_id]
term_genes = {g: 1 for g in term_info['genes']}  # $ref for simulation of v1{$term}

num_in_C = 0
num_in_O = 0
gene_list1 = {}  # Simulate v1%gene_list1
...
print(f"Gene_total (number of genes matching): {gene_total}")
